...
- 字母数字变长字段（**批号 10、序列号 21、企业内部 90–99** 等）仅接受
  **ASCII 0x20–0x7E**（GS1 82 字符集，含空格与标点）；
  emoji、NUL、TAB、换行、DEL、中文/全角字符等一律**定位到该字符、稳定拒绝**，
  错误响应带机器可读代码 `"code":"unsupported_character"`，且**不产生批次信息**。
- **空格含身份**：字段末尾的空格是值的一部分，绝不与扫码枪后缀一起删除
  （`"INK2407 "` 与 `"INK2407"` 是两个不同批号，两种格式一致）。
- 只按既有约定容忍真正的 **AIM 符号标识符前缀**（如 `]d2`）、字段间 **FNC1(GS)**
  与整个标签最末尾的 **CR/LF 行尾**；末尾 TAB 不是行尾，与字段中部 TAB 同样拒绝。
- 长度只按**可编码标签内容**计数：字符集外的多字节字符在长度检查前即被拒绝，
  超出实际承载边界的数据不会显示“已识别”。
- 前端把原文中的控制字符以控制图形符号呈现（␀ NUL、␉ TAB、␊ LF、␍ CR、
  ␝ FNC1、␡ DEL），已识别值中的空格显示为 ␠，精确原值保留在 `data-value`，
  收料员可逐字符核对数据库所对应的原标签。
...
# → {"ok":false,"code":"parse_error","message":"标签解析失败：商品编码校验位错误…","position":17,...}

# 批号含 GS1 字符集外字符（如下例 TAB）：code=unsupported_character，
# position 指向该字符；尾随空格则是值本身（"INK2407 " 与 "INK2407" 不同）
curl -s -X POST http://localhost:8001/api/gs1-label \
  -H 'Content-Type: application/json' \
  --data-binary '{"raw":"(01)09506000134352(17)280930(10)ABCD\t"}'
# → {"ok":false,"code":"unsupported_character","position":36,...}
```
...
3. **Playwright**：真实浏览器经 nginx 访问 FastAPI，覆盖放行、超差、端点值、
   ...
   另有 GS1 专项：末尾空格（两种格式下均保留身份）、字段中部与末尾控制符
   （TAB/NUL/换行，`code=unsupported_character` 与位置逐项核对）、
   emoji/中文/全角空格等多字节字符、空格/控制字符在页面的可见文本
   （␠/␉/␀/␊），以及合法 ASCII 标签、AIM 前缀、FNC1、CR/LF 行尾的兼容性。
