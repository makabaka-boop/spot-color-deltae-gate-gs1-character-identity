...
#      尾随空格身份、TAB/NUL/换行/emoji 等字符集外字符的 unsupported_character 拒绝与定位、
#      按可编码内容计数、AIM 前缀/FNC1/CRLF 行尾容忍与既有 ASCII 标签兼容）
#   2) Vitest：前端输入校验与旧结论清除；标签核验区状态机、错误代码与定位、
#      空格␠/控制符␉␀␊ 的页面可见化（组件级，fetch 打桩）
